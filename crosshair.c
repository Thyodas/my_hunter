...
void create_crosshair(game_data_t *g_data)
{
    ...
    sprite_crosshair->texture = sfTexture_createFromImage(sprite_crosshair->image, NULL);
    sprite_crosshair->sprite = sfSprite_create();
    sfSprite_setTexture(sprite_crosshair->sprite, sprite_crosshair->texture, sfTrue);
